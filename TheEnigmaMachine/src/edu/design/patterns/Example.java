...
public class Example {
	...
	{
		EnigmaMachine machine = new Modelo1();
	...
	{
		EnigmaMachine machine = new Modelo2();
		GermanAgent sigfrid = new GermanAgent("Sigfrid");
		Message m = new Message(sigfrid.getName(), "Hanz", "Wir sind in den Ofen. Jemand nahm die Katze Milch");
		sigfrid.SendMessage(m, machine);
	}
	...
	{
		EnigmaV2 machine = new EnigmaV2();
		
		Cifrador cifradorAvanzado = new CifradorAvanzado();
		machine.setCifrador(cifradorAvanzado); //pongo el cartucho
		
		GermanAgent sigfrid = new GermanAgent("Sigfrid");
		Message m = new Message(sigfrid.getName(), "Hanz", "Wir sind in den Ofen. Jemand nahm die Katze Milch");
		sigfrid.SendMessageEnigmaV2(m, machine);
		
		Cifrador cifradorCesar = new CifradorCesar();
		machine.setCifrador(cifradorCesar); //cambio el cartucho
